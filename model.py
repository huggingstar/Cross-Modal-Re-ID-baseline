...
class Normalize(nn.Module):
    ...
    def forward(self, x):
        # pow(power)Tensor的每个元素的power次方
        # sum(1:按行求和, keepdim=True:计算结果保持和input Tensor一致的size())
        norm = x.pow(self.power).sum(1, keepdim=True).pow(1. / self.power)
        # out中的元素除以norm中对应位置的元素
        out = x.div(norm)
        return out
...
class Non_local(nn.Module):
    ...
    def forward(self, x):
        ...
        # 查看size()返回值中从右往左的第一个结果
        N = f.size(-1)
        # f_div_C = torch.nn.functional.softmax(f, dim=-1)
        f_div_C = f / N

        y = torch.matmul(f_div_C, g_x)
        # 通过permute()操作后的数据在内存中不一定是连续的
        # contiguous()会把原来的数据拷贝一份到地址连续的内存中
        # Tensor.view(...)的前提是TensorFlow的内存地址是连续的
...
class embed_net(nn.Module):
    def __init__(self, class_num, no_local='on', gm_pool='on', arch='resnet50'):
        ...
        # Module.apply(fn) 会递归地将用fn应用到父Module的每个子Module，以及父Module自身
        # apply(fn) 通常用于对Module进行初始化
        self.bottleneck.apply(weights_init_kaiming)
        self.classifier.apply(weights_init_classifier)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.gm_pool = gm_pool

    # x1: visible       x2:thermal
