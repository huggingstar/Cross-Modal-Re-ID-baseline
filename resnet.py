...
class ResNet(nn.Module):
    '''
    使用Bottleneck 构造 Resnet

    :parameter block: Bottleneck
    :parameter layers: 整数list。legnth为层数， 每个元素是每层中包含的Bottleneck的个数
    '''
    # model = ResNet(Bottleneck, [3, 4, 6, 3], **kwargs)
    # **kwargs: last_conv_stride=1, last_conv_dilation=1
    ...
    # layers = [3, 4, 6, 3]
    #   _make_layer(      block, 64,     layers[0]=3)
    #   _make_layer(      block, 128,    layers[1]=4, stride=2)
    #   _make_layer(      block, 256,    layers[2]=6, stride=2)
    #   _make_layer(      block, 512,    layers[3]=3, stride=last_conv_stride,
    #                                              dilation=last_conv_dilation)
    #
    # blokc = Bottleneck


    '''使用多个 Bottleneck 来构造一个新的层'''
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1):
        '''
        使用多个 Bottleneck 来构造一个新的层

         :parameter block: Bottleneck
         :parameter planes: 输出维度
         :parameter blocks: 当前层包含几个Bottlenec
         '''
        downsample = None
        #                self.inplanes = 64    Bottleneck.expansion = 4
